...
jest.mock('@/components/ui/ImageCropper', () => {
  return {
    __esModule: true,
    default: ({ onCancel, onCropComplete }: { 
      onCancel: () => void;
      onCropComplete: (file: File) => void;
    }) => {
      return (
        <div data-testid="mock-image-cropper">
          <button onClick={() => onCancel()} data-testid="cancel-crop-button">
            Cancel
          </button>
          <button 
            onClick={() => onCropComplete(new File(['test'], 'cropped-test.png', { type: 'image/png' }))} 
...
jest.mock('@/components/ui/FileUpload', () => {
  return {
    __esModule: true,
    default: ({ onChange, value, buttonText }: { 
      ...
      return (
        <div data-testid="mock-file-upload">
          ...
          {value && typeof value === 'string' && (
            <img src={value} alt="Preview" data-testid="preview-image" />
          )}
        </div>
      );
    }
...
describe('AvatarUpload Component', () => {
  ...
  it('renders correctly with existing avatar URL', () => {
    ...
    expect(screen.getByTestId('preview-image')).toHaveAttribute('src', mockAvatarUrl);
  });
  ...
  it('handles upload errors', async () => {
    ...
    (updateUserAvatarV2 as jest.Mock).mockRejectedValue(new Error(errorMessage));
    ...
    expect(screen.queryByText('Avatar updated successfully')).not.toBeInTheDocument();
  });
}); 