...
describe("EmailSection", () => {
  ...
  it("updates local state immediately after email change", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByText("verified@example.com"),
      ).not.toBeInTheDocument();
  ...
  it("handles errors when update email fails", async () => {
    ...
    (updateUserEmail as jest.Mock).mockRejectedValueOnce(
  ...
  it("handles errors when resend verification fails", async () => {
    (resendUserVerificationEmail as jest.Mock).mockRejectedValueOnce(
  ...
  it("updates component state when user prop changes", () => {
    const { rerender } = render(
      <EmailSection user={mockUser} onError={mockOnError} />,
    );
