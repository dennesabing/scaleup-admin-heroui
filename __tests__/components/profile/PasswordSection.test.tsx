...
describe('PasswordSection', () => {
  ...
  it('validates that new password must be different from current password', async () => {
    ...
    const submitButton = screen.getByRole('button', { name: /update password|no changes/i });
    expect(submitButton).toBeDisabled();
  });
  ...
  it('handles validation in the submit handler', async () => {
    ...
    const button = screen.getByRole('button', { name: /update password|no changes/i });
  ...
  it('handles errors when update password fails', async () => {
    ...
    (updateUserPassword as jest.Mock).mockRejectedValueOnce(new Error(errorMessage));
  ...
  it('clears error messages when user types', async () => {
    ...
    await user.type(screen.getByLabelText('New Password'), 'long_enough_password');
