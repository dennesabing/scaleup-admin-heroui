...
describe('TeamContext', () => {
  ...
  it('shows loading state and loads teams when organization is available', async () => {
    ...
    expect(localStorageMock.setItem).toHaveBeenCalledWith('current_team_id', '1');
  });
  ...
  it('handles errors when loading teams', async () => {
    (getOrganizationTeams as jest.Mock).mockRejectedValue(new Error('Network error'));
    ...
    await waitFor(() => {
      expect(screen.queryByText('Loading...')).not.toBeInTheDocument();
      expect(screen.getByText('Error: Failed to load teams. Please try again later.')).toBeInTheDocument();
    });
  });
  ...
  it('allows changing the current team', async () => {
    ...
    expect(localStorageMock.setItem).toHaveBeenCalledWith('current_team_id', '2');
  });
