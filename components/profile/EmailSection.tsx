...
export function EmailSection({
  ...
  const formStatus = useMemo(() => {
    ...
    const isDifferentEmail =
      emailForm.email.toLowerCase() !== currentEmail.toLowerCase();

    // Check if form has all required fields
    const isComplete =
      emailForm.email.length > 0 && emailForm.password.length > 0;
  ...
  return (
    <div className="bg-background rounded-lg p-6">
      ...
      <p className="text-default-500 mb-4">
        Update your email address. A verification link will be sent to your new
        email.
      </p>
      ...
      <form onSubmit={handleSubmit}>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium" htmlFor="new_email">
              New Email
            </label>
            <Input
              required
