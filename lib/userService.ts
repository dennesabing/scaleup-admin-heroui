...
export const updateUserAvatar = async (file: File): Promise<string> => {
  try {
    ...
    const avatarUrl = response.data.avatar_url || response.data.user?.profile?.avatar_url;
...
export const updateUserAvatarV2 = async (file: File): Promise<string> => {
  try {
    ...
    const response = await axiosInstance.post(USER_ENDPOINTS.AVATAR_V2, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
