...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!token || typeof token !== 'string' || !organization_id || typeof organization_id !== 'string') {
    return res.status(400).json({ message: 'Invalid token or organization ID' });
  }

  try {
    // Forward the complete registration request to the backend API
    const response = await axiosInstance.post(`/invitations/${organization_id}/${token}/accept`, req.body);
