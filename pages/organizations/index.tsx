...
export default function OrganizationsPage() {
  ...
  const canCreateOrganization =
    currentUser?.roles?.includes("Organization Head");
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="text-center p-8 bg-default-100 rounded-lg">
          <BuildingIcon className="mx-auto mb-4 text-default-400" size={48} />
          ...
          {canCreateOrganization && (
            <Button color="primary" onClick={handleCreateOrganization}>
              Create Organization
            </Button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {organizations.map((org) => (
            <Card
              key={org.id}
              isPressable
              onPress={() => handleViewOrganization(org.id)}
            >
